...
```mermaid
graph LR
    A[Attacker places siege banner] --> B[Siege period begins]
    B --> C[Both sides fight near the banner]
    C --> D{Who controls the banner?}
    D -->|Attackers win| E[Town captured / plundered]
    D -->|Defenders win| F[Siege repelled]
```
